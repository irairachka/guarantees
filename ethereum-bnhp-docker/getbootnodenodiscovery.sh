...
ENODE_LINE=$(docker logs ethereum-bootnode-static 2>&1 |  grep enode:// | grep enode | head -n 1 | awk -F  "enode://" '{print $2}' | awk -F  '[::]' '{print $1$4} ' |  awk -F  [ '{print $1"'$DOCKER_IP':"$2} '   | awk -F  "?" '{print $1}'  )
